...
def index(request):
    ...
    return render(request, 'market/index.html', {'featured': featured})

def project_list(request):
    listings = CreditListing.objects.filter(available=True).order_by('-created_at')
    return render(request, 'market/project_list.html', {'listings': listings})

def project_detail(request, pk):
    project = get_object_or_404(Project, pk=pk)
    listings = project.listings.filter(available=True)
    return render(request, 'market/project_detail.html', {'project': project, 'listings': listings})
...
def project_onboard(request):
    if request.method == 'POST':
        ...
        p_form = ProjectForm()
        l_form = ListingForm()
    return render(request, 'market/project_onboard.html', {'p_form': p_form, 'l_form': l_form})
...
def buyer_register(request):
    ...
    return render(request, 'market/buyer_register.html', {'form': form})
...
def cart_view(request):
    ...
    for lid, qty in cart.items():
        ...
        total += amount
    return render(request, 'market/cart.html', {'items': items, 'total': total})
...
def payment_success(request, txn_id):
    ...
    return render(request, 'market/checkout.html', {'txn': txn})

@login_required
def dashboard(request):
    projects = request.user.projects.all()
    txns = request.user.transactions.all()
    return render(request, 'market/dashboard.html', {'projects': projects, 'txns': txns})
...
def certificate_view(request, txn_id):
    ...
    return render(request, 'market/certificate.html', {'txn': txn})
